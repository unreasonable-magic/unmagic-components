...
Extract Toybox image sampling into a standalone component which updates an ordinary color field.

`image_color_picker src, alt:, input:`. Other HTML options go on the custom element. Blank sources raise ArgumentError.
...
`unmagic-image-color-picker` adds interaction. Listeners on the element are installed once.
