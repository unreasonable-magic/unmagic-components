...
Move and resize a crop, choose a fixed aspect or circle, and export a PNG locally.

`image_crop src, alt:, aspect: nil, circular: false, name: nil`. Other HTML options go on the custom element. Blank sources raise ArgumentError.
...
`unmagic-image-crop` adds interaction. Listeners on the element are installed once.
